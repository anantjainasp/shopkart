...
const Checkout = () => {
  ...
  const handleAddressSelect = (e) => {
    ...
    if (selectedValue === 'new') {
      ...
      const selectedAddress = addresses.find(addr => addr.street === selectedValue);
      if (selectedAddress) {
        ...
        <ul className="space-y-4">
          {productDetails && productDetails.length > 0 ? productDetails.map(item => (
            <li key={item.id} className="flex justify-between items-center border-b pb-2">
              ...
              <span className="text-lg font-medium text-gray-900">x {item.quantity}</span>
            </li>
          )) : <p>No items in cart</p>}
        </ul>
        <p className="text-2xl font-bold text-gray-800 mt-6">Total: {productDetails && productDetails.length > 0 ? `$${productDetails.reduce((total, item) => total + item.price * item.quantity, 0).toFixed(2)}` : '$0.00'}</p>
      </div>
      <div className="mt-6">
        <h3 className="text-xl font-bold text-gray-800 mb-4">Shipping Address</h3>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {addresses.map((addr, index) => (
            <div key={index} className="p-4 border rounded-lg shadow-md bg-white flex items-center">
              <input
                type="radio"
                name="address"
                value={addr.street}
                ...
                className="mr-2"
              />
              <span className="text-lg font-medium text-gray-800">{addr.street}, {addr.city}, {addr.state}, {addr.zipCode}, {addr.country}</span>
            </div>
          ))}
          <div className="p-4 border rounded-lg shadow-md bg-white flex items-center">
            <input
              ...
              className="mr-2"
            />
            <span className="text-lg font-medium text-gray-800">Add New Address</span>
          </div>
        </div>
        {editAddressMode && (
          <div className="mt-4">
